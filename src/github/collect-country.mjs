...
async function searchCandidateLogins(graphql, query, maxResults = 100) {
...
function toDeveloperRecord(user, yearWindows, config) {
  const normalizedLocation = normalizeCountryLocation(user.location, config);
...
export async function collectCountry({
  token,
  countryCode,
  now = new Date(),
  concurrency = 4,
  ...
  if (!config) throw new Error(`Unsupported country code: ${countryCode}`);

  const graphql = createGraphQLClient({ token });
  const candidateLogins = new Set();

  for (const query of countrySearchQueries(config)) {
    if (candidateLogins.size >= maxCandidates) break;
    const remaining = maxCandidates - candidateLogins.size;
    const discovered = await searchCandidateLogins(graphql, query, Math.min(remaining, 100));
  ...
  const enriched = await mapWithConcurrency(logins, concurrency, async (login) => {
    ...
    return data.user ? toDeveloperRecord(data.user, yearWindows, config) : null;
  });

  const developers = enriched.filter(Boolean);
  return {
    country: { code: config.code, name: config.name },
